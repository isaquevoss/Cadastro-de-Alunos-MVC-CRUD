...
uses
  uHelpers, FireDAC.Phys.SQLiteWrapper;
...
begin
  ...
  Grid.Cells[3, _Row] := _Aluno.turma;
  Grid.Cells[4, _Row] := FormatDateTime('dd/mm/yyyy', _Aluno.data_nascimento);
end;
...
begin
  ...
  Grid.Cells[0, 0] := 'Id';
  Grid.Cells[1, 0] := 'Nome';
  Grid.Cells[2, 0] := 'Sobrenome';
  Grid.Cells[3, 0] := 'Turma';
  Grid.Cells[4, 0] := 'Data de Nascimento';
...
begin
  if ARow > 0 then
    AlunoSelecionado := TAluno(Grid.Objects[0, ARow]);  // Aqui estamos sempre atualizando o aluno selecionado ao mudar de linha no grid;
end;
